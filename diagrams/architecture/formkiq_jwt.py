...
with Diagram("FormKiQ JWT Authorization", show=False, graph_attr=graph_attr):

    with Cluster("Users", graph_attr={"margin": "10"}):
        users = Users("Users")

    with Cluster("API Gateway", direction="TB", graph_attr={"margin": "10"}):
        api = APIGateway("JWT Authorization") 
    ...
    users >> Edge(ltail='cluster_Applications',lhead='cluster_API Gateway') >> api
